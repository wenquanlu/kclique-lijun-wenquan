...
printf("|%u| %.1f| %lu", k, alpha, N);
    ...
    vector<v_size> nodes;
    v_size sz = 0; // sz is the number of nodes in S.
    nodes.resize(g->vCnt/4);

    for(v_size u = st; u < ed; u++) { // for (v_size u = 0, u < vCnt; u++) iterate over all vertices
        v_size outDegree = g->pIdx[u+1]-g->pIdx2[u];

        if(outDegree + 1 < k) continue; // when number of neighbours + 1 < k, impossible to form a k-clique
        // buildSubGraphFalse(u);
        buildSubGraphTrue(u); // build subgraph as subG

        if(subG->eCnt / 2 > alpha* (k-1) * subG->vCnt) { // alpha is set to 1.0 by default
            if(sz == nodes.size()) {
                nodes.resize(sz + ed - u); // make nodes size larger
            }
            nodes[sz] = u; // add node to S, "the node in dense region"
            sz++;
        }
        else { ////////////////////////// exact counting starts //////////////////////////
...
void samplePlusExact::previousWork() {
    ...
    for(v_size u = 0; u < g->vCnt; u++) {
        ...
        hashTable[u].build(g->pEdge + g->pIdx[u], // address of u's edge in pEdge
            g->pIdx[u + 1] - g->pIdx[u]); // number of in and out neighbours of u
    }
...
/*This function doesn't build the subgraph, rather only counts the number edges and vertices in the subgraph*/
void samplePlusExact::buildSubGraphFalse(v_size u) {
    v_size outDegree = g->pIdx[u+1]-g->pIdx2[u];

    v_size st = g->pIdx2[u], l = 0;
    for(v_size i = st; i < g->pIdx[u+1]; i++) {
        v_size v = g->pEdge[i]; // v is a neighbour of u

        for(v_size j = st; j < g->pIdx[u+1]; j++) {
            v_size w = g->pEdge[j]; // w is a neighbour of u
            if(cc(v, w)) l++; // if v and w are adjacent, the increment l, count for number of edges in the subgraph
        }
    }
...
/*build subgraph generated by neighbours of u
  The subgraph is stored as subG
  pidx has size ~ number of outneighbours of u + 1
  */
void samplePlusExact::buildSubGraphTrue(v_size u) {
    v_size outDegree = g->pIdx[u+1]-g->pIdx2[u]; //out-degree of u, the number of vertices in subgraph

    v_size st = g->pIdx2[u], l = 0; // base index of outneighbour edge
    subG->pIdx[0] = 0;
    subG->pIdx2[0] = 0;

    for(v_size i = st; i < g->pIdx[u+1]; i++) { 
        v_size v = g->pEdge[i]; // v is each outneighbour of u
        ...
        for(v_size j = st; j < g->pIdx[u+1]; j++) {
            ...
            if(i == j) {
                ...
                subG->pEdge[l++] = j - st; // set neighbour as updated value
                subG->pIdx[i - st + 1]++;
            }
        }
    }
